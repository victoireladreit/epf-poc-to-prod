...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    dataset = LocalTextCategorizationDataset(dataset_path, train_conf['batch_size'],
                                             train_ratio=0.8, min_samples_per_label=train_conf['min_samples_per_label'],
    ...
    model.add(
        Dense(train_conf["dense_dim"], activation='relu', input_dim=768))
    ...
    os.makedirs(artefacts_path, exist_ok=True) # be sure directory exists before adding files to it
    ...
    labels_index = dataset.get_index_to_label_map() # model is going to give an index, make sense of it by applying
    # corresponding label
